...
//				});
			...
			orderVerify:function(){
				...
				var yzName=true,yzCompany=true,yzPhone=true,yzZip=true,yzAddress=true,yzArea=true;
				//用户名
				...
				var orderAreaErr=$('#orderAreaErr');
				$(orderArea).on('blur',function(){
					if(!orderArea.val()){
						orderAreaErr.html('请输入地区');
						yzArea=false;
					}else{
						orderAreaErr.html('');
						yzArea=true;
				...
				$(orderNumber).on('keyup',function(evt){
					var numberVal=$('#number').val();
					numberVal = numberVal.replace(/[^\d]/g, '');
					numberVal = parseInt(numberVal, 10);
					if(isNaN(numberVal) || !numberVal){
						numberVal = 1;
					}
					this.value = numberVal;						
					var totalPrice=numberVal*299;
